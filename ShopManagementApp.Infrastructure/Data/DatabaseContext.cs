...
namespace ShopManagementApp.Infrastructure.Data;
...
public partial class DatabaseContext : IDatabaseContext, IAsyncDisposable
{
    ...
    public async Task SetContext()
    {
        ...
        try
        {
            ...
            if (_userSessionContext.Tenant_ID != default)
            {
                ...
                await _sqlConnection.ExecuteAsync("EXEC sp_set_session_context @key=N'TENANT_ID', @value=@TENANT_ID;", tenantParameters, _sqlTransaction).ConfigureAwait(false);
            ...
            await _sqlConnection.ExecuteAsync("EXEC sp_set_session_context @key=N'BYPASS_RLS', @value=@BYPASS_RLS;", bypassRlsParameters, _sqlTransaction).ConfigureAwait(false);
    ...
    public async Task CommitAsync()
    {
        ...
        try
        {
            if (_sqlTransaction != null)
            {
                await _sqlTransaction.CommitAsync().ConfigureAwait(false);
    ...
    public async Task RollbackAsync()
    {
        ...
        try
        {
            if (_sqlTransaction != null)
            {
                await _sqlTransaction.RollbackAsync().ConfigureAwait(false);
    ...
    {
        if (_sqlConnection.State is ConnectionState.Closed or ConnectionState.Broken)
        {
