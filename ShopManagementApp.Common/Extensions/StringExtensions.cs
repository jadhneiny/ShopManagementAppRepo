...
namespace ShopManagementApp.Common.Extensions;

public static partial class StringExtensions
{
    #region Regex Definition
    [GeneratedRegex(@"^\w+", RegexOptions.Compiled)]
    private static partial Regex ExtractFirstWordRegex();

    [GeneratedRegex(@"[-\s]", RegexOptions.Compiled)]
    private static partial Regex ConvertSpaceAndDashToUnderscoreRegex();

    [GeneratedRegex(@"(\B[A-Z])", RegexOptions.Compiled)]
    private static partial Regex DetectUpperCaseBoundaryRegex();

    [GeneratedRegex(@"([\p{Ll}\d])([\p{Lu}])", RegexOptions.Compiled)]
    private static partial Regex AddUnderscoreBeforeCapitalLetterRegex();

    [GeneratedRegex(@"(^[a-z])|\.\s+(.)", RegexOptions.ExplicitCapture)]
    private static partial Regex SentenceCaseRegex();

    [GeneratedRegex(@"([\p{Lu}]+)([\p{Lu}][\p{Ll}])", RegexOptions.Compiled)]
    private static partial Regex SplitCamelCaseRegex();

    [GeneratedRegex(@"[A-Z]+(?![a-z])|[A-Z][a-z]*|\d+", RegexOptions.Compiled)]
    private static partial Regex ExtractWordsFromMixedCaseRegex();
    #endregion
    #region Repeat
    ...
    public static string ToCamelCase(this string inputString)
    {
        ...
        var words = ExtractWordsFromMixedCaseRegex().Matches(inputString)
                        .Cast<Match>()
                        .Select(m => m.Value)
                        .ToList();
    ...
    {
        return DetectUpperCaseBoundaryRegex().Replace(inputString, "_$1").ToLowerInvariant();
    }
    public static string ToKebabCase(this string inputString)
    {
        return DetectUpperCaseBoundaryRegex().Replace(inputString, "-$1").ToLowerInvariant();
    }
    ...
    public static string ToSentenceCase(this string inputString)
    {
        ...
        return SentenceCaseRegex().Replace(lowerCase, s => s.Value.ToUpperInvariant());
    }
    public static string NormalizeToSnakeCase(this string inputString)
    {
        var handleCamelCase = SplitCamelCaseRegex().Replace(inputString, "$1_$2");
        var addUnderscoreBeforeCapital = AddUnderscoreBeforeCapitalLetterRegex().Replace(handleCamelCase, "$1_$2");
        var replaceSpacesAndDashes = ConvertSpaceAndDashToUnderscoreRegex().Replace(addUnderscoreBeforeCapital, "_");
    ...
    {
        var match = ExtractFirstWordRegex().Match(input);
